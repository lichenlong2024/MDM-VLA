...
class GenerateConfig:
    ...
    use_moe_action_head: bool = True                 # If True, uses MoE action head instead of L1 regression head
    use_action_id_discriminator: bool = True         # If True, uses Action ID discriminator
...
def initialize_model(cfg: GenerateConfig):
    ...
    if cfg.use_action_id_discriminator:
        ...
        action_id_discriminator = action_id_discriminator.to(torch.bfloat16).to("cuda:0" if torch.cuda.is_available() else "cpu")
...
def run_episode(
    ...
    while t < max_steps + cfg.num_steps_wait:
        ...
        robot_info["ee_pos"] = robot_info["ee_pos"]
        #print(f"机械臂末端位置: {robot_info['ee_pos']}, 模型预测动作基元: {action_id_pred}")
        if done:
            success = True
            break
        t += 1
...
def eval_libero(cfg: GenerateConfig) -> float:
    ...
    model, action_head, proprio_projector, noisy_action_projector, processor, action_id_discriminator = initialize_model(cfg)
    import pdb;pdb.set_trace()
