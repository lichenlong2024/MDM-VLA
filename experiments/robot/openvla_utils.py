...
def get_vla_action_with_action_id(
        cfg: Any,
        vla: torch.nn.Module,
        processor: Any,
        obs: Dict[str, Any],
        ...
        proprio_projector: Optional[torch.nn.Module] = None,
        noisy_action_projector: Optional[torch.nn.Module] = None,
        action_id_discriminator: Optional[torch.nn.Module] = None,
    ...
    with torch.inference_mode():
        ...
        if action_head is None:
            ...
            # Custom action head for continuous actions
            action, actions_hidden_states, action_id_pred = vla.predict_action(
                **inputs,
                unnorm_key=cfg.unnorm_key,
                do_sample=False,
                proprio=proprio,
    ...
    # Extract subset of actions for open loop steps
    return [action[i] for i in range(min(len(action), cfg.num_open_loop_steps))], action_id_pred
