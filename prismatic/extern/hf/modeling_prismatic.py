...
class OpenVLAForActionPrediction(PrismaticForConditionalGeneration):
    ...
    def _regression_or_discrete_prediction_with_action_id(
            self,
            pixel_values,
            input_embeddings,
            all_actions_mask,
        ...
        action_queries = action_queries.view(1, action_queries.shape[0], action_queries.shape[1]).repeat(
            input_embeddings.shape[0], 1, 1)  # (b, chunk_size, h)
        # Replace action token embeddings with noisy action embeddings
        input_embeddings = self._replace_input_embeddings(input_embeddings.clone(), all_actions_mask,
                                                          action_queries)
        ...
        multi_layer_hidden_states = torch.cat(multi_layer_hidden_states, dim=1)
        task_features = language_model_output.hidden_states[-1][:, :NUM_PATCHES, :].to(torch.bfloat16)  # [B, num_patches, D]

        # Action特征是动作token的隐藏状态
        # 我们需要从文本隐藏状态中提取动作相关的部分
        text_hidden_states = language_model_output.hidden_states[-1][:, NUM_PATCHES:-1, :]  # [B, text_len, D]
        action_features = text_hidden_states[:,
                                    NUM_PATCHES + NUM_PROMPT_TOKENS: NUM_PATCHES + NUM_PROMPT_TOKENS + NUM_TOKENS,
                                    :, ].reshape(batch_size, -1, text_hidden_states.shape[-1]).to(torch.bfloat16)

        if action_id_discriminator is not None:
            action_probs, _, _ = action_id_discriminator(
                pixel_values=pixel_values,
                proprio=proprio,
                input_ids= None,
        ...
        if action_head is not None:
            ...
            normalized_actions = action_head.predict_action(multi_layer_hidden_states,
                                                            proprio=proprio,
                                                            proprio_projector=proprio_projector,
                                                            action_probs=action_probs,  # 现在这个参数接收的是动作概率 (B, num_action_ids)
                                                            use_moe=use_moe_action_head,
                                                            )
            normalized_actions = normalized_actions.reshape(NUM_ACTIONS_CHUNK, ACTION_DIM)
            normalized_actions = normalized_actions.float().cpu().detach().numpy()
            #import pdb;pdb.set_trace()
            return normalized_actions, actions_hidden_states,torch.argmax(action_probs, dim=-1)
        else:
            # Discrete token-based prediction
            predicted_action_token_ids = (
                language_model_output.logits[
    ...
    def predict_action(
        ...
        **kwargs: str,
        ...
        if action_id_discriminator is not None:
            normalized_actions, actions_hidden_states ,action_id_pred = self._regression_or_discrete_prediction_with_action_id(
                pixel_values,
                input_embeddings,
                all_actions_mask,
                projected_patch_embeddings,
                ...
                use_moe_action_head = use_moe_action_head,
                )
        else:
        ...
        return actions, actions_hidden_states, action_id_pred
