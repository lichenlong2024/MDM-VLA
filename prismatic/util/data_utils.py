...
class PaddedCollatorForActionPrediction:
    ...
    def __call__(self, instances: Sequence[Dict[str, torch.Tensor]]) -> Dict[str, torch.Tensor]:
        ...
        action_ids = None
        if "action_id" in instances[0]:

            action_ids = [instance["action_id"] for instance in instances]
            # 如果 action_id 是标量张量，直接堆叠
            if isinstance(action_ids[0], torch.Tensor) and action_ids[0].dim() == 0:
                action_ids = torch.stack(action_ids)
            # 如果是普通数字，转换为张量后堆叠
            elif isinstance(action_ids[0], (int, float)):
                action_ids = torch.tensor(action_ids)
            # 如果已经是张量，直接堆叠
            else:
                action_ids = torch.stack(action_ids)
